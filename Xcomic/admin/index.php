...
class XcomicAdminDriver {
	...
	function postNews() {
		global $message;
		$newsTitle=(!empty($_REQUEST['title'])) ? $this->security->allowOnlyWords($_REQUEST['title']) : NULL;
		$newsCategory=(!empty($_REQUEST['category'])) ? $this->security->allowOnlyWords($_REQUEST['category']) : 'default'; //Default
		$newsContent=(!empty($_REQUEST['content'])) ? $this->security->allowBasicWriting($_REQUEST['content']) : NULL;
	...
	function postComic() {
		global $message;
		$comicTitle=(!empty($_REQUEST['title'])) ? $this->security->allowOnlyWords($_REQUEST['title']) : NULL;
	...
	function addUser() {
		...
		$username=(!empty($_REQUEST[ADDIN_USERNAME])) ? $this->security->allowOnlyWords($_REQUEST[ADDIN_USERNAME]) : NULL;
		$password=(!empty($_REQUEST[ADDIN_PASSWORD])) ? $this->security->allowOnlyWords($_REQUEST[ADDIN_PASSWORD]) : NULL;
		$email=(!empty($_REQUEST[ADDIN_EMAIL])) ? $this->security->allowEmail($_REQUEST[ADDIN_EMAIL]) : NULL;
	...
	function deleteUser() {
		...
		$username=(!empty($_REQUEST[GETIN_USERNAME])) ? $this->security->allowOnlyWords($_REQUEST[GETIN_USERNAME]) : NULL;
		
		//Check for error
		if(empty($username))
			$message->error('The username was left blank.');
	...
	function changeComicStatus() {
		...
		$comments=(!empty($_REQUEST[NEXT_COMIC_COMMENT])) ? $this->security->allowBasicWriting($_REQUEST[NEXT_COMIC_COMMENT]) : NULL;
